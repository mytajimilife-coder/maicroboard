...
try {
    ...
    if ($stmt->rowCount() == 0) {
        $db->exec("ALTER TABLE mb1_board_config ADD COLUMN bo_use_search TINYINT(1) NOT NULL DEFAULT 1");
    }
} catch (Exception $e) {
    // 오류 무시 (권한 문제 등)
}
...
<div class="admin-card">
    ...
    <form method="post">
      ...
      <div class="form-grid">
          ...
          <div class="form-group" style="display: flex; align-items: flex-end; padding-bottom: 0.75rem; gap: 2rem; flex-wrap: wrap;">
            ...
            <label class="plugin-checkbox" style="margin-bottom: 0;">
              <input type="checkbox" name="bo_use_search" <?php echo ($board['bo_use_search'] ?? 1) ? 'checked' : ''; ?>>
              <?php echo $lang['use_search_label'] ?? '통합검색 반영'; ?>
            </label>
          </div>
      </div>
